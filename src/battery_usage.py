...
_CHARGE_PAT = _re.compile(
    _TIMESTAMP_PAT_STR + r'.*?Using (?P<type>AC|Batt|BATT).*?\(Charge:\s*(?P<charge>\d+)',
    _re.IGNORECASE
)
_DISPLAY_PAT = _re.compile(_TIMESTAMP_PAT_STR + r'.*?Display is turned (?P<state>\w+)', _re.IGNORECASE)
...
_GREP_ARGS = ('grep', '-e', '[[:blank:]]Using[[:blank:]]', '-e', '[[:blank:]]Display is turned[[:blank:]]')
...
def pmset_log_proc() -> _subprocess.Popen:
    ...
    grep_proc = _subprocess.Popen(_GREP_ARGS, encoding="UTF-8", stdin=pmset_proc.stdout, stdout=_subprocess.PIPE)
    return grep_proc
